...
const App: React.FC = () => {
  ...
  const handleAddTask = useCallback(async (taskData: Omit<Task, 'id'>, actionItemsData: Omit<ActionItem, 'id' | 'taskId' | 'meetingNoteId'>[], onSuccessCallback?: () => void) => {
    try {
        ...
        const newActionItems = actionItemsData.map(ai => ({ ...ai, id: `ai-${new Date().toISOString()}-${Math.random()}`, taskId: savedTask.id }));
  ...
  const handleSubmitReview = useCallback(async (taskId: string, comment: string, mentions: string[]) => {
      ...
      } catch (error) { console.error("Lỗi gửi duyệt:", error); }
  }, [tasks, currentUser]);
  ...
  const handleApprovalAction = useCallback(async (taskId: string, action: 'approve' | 'reject', comment?: string) => {
      ...
      try {
          await taskService.updateTask(taskId, updates);
          setTasks(prev => prev.map(t => t.id === taskId ? { ...t, ...updates } : t));
      } catch (error) { console.error("Lỗi duyệt bài:", error); }
  }, [tasks, currentUser]);
  ...
  const handleUpdateTask = useCallback(async (taskId: string, updates: Partial<Task>) => { 
      try {
          await taskService.updateTask(taskId, updates);
          setTasks(prev => prev.map(t => t.id === taskId ? {...t, ...updates} : t)); 
      } catch (error) { console.error("Lỗi update task:", error); }
  }, []);
  ...
  const handleUpdateTaskPlanning = useCallback(async (taskId: string, newPlan: { week?: string; month?: string; quarter?: string }) => { 
      try {
          ...
          await taskService.updateTask(taskId, updates as any);
          setTasks(prevTasks => prevTasks.map(task => { if (task.id === taskId) { return { ...task, ...updates }; } return task; })); 
          const task = tasks.find(t => t.id === taskId); 
          if (task) { 
              let message = `Task "${task.name}" `; 
              if (newPlan.week) message += `được lên kế hoạch cho tuần ${newPlan.week}.`; 
              else if (newPlan.month) message += `được lên kế hoạch cho tháng ${newPlan.month}.`; 
              else if (newPlan.quarter) message += `được lên kế hoạch cho quý ${newPlan.quarter}.`; 
              else message += `được trả về Backlog.`; 
              const newLog: LogEntry = { id: `log-${Date.now()}`, timestamp: new Date().toISOString(), message: message, taskId: taskId }; 
              addLogEntries([newLog]); 
          } 
      } catch (error) { console.error("Lỗi update planning:", error); }
  }, [tasks, addLogEntries]);
